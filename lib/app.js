...
function createApp( cfg ) {
	...
	let lastLoaded = null; // the last sync report — stockLanes() reads the loaded methods files off it
	...
	/**
	 * R6 — the ASSISTANT lanes (`hint`/`propose`) over the WHOLE room: EVERY loaded
	 * `kind:'methods'` bundle contributes its certified shapes (the audit fix — `methods[0]`
	 * silently dropped the rest of the room). Null when no method is certified (an empty
	 * referential would refuse everything — better no lane than a lying gate).
	 */
	function stockLanes() {
		const files = (lastLoaded && lastLoaded.methods) || [];
		const all = [];
		for ( const f of files ) {
			try {
				const art = JSON.parse(fs.readFileSync(path.join(sgcDir, f), 'utf8'));
				(art.methods || []).forEach(( m ) => all.push(m) );
			} catch ( e ) { /* already reported skipped by sync() */ }
		}
		if ( !all.length ) return null;
		return require('skynet-graph/lib/sg/mcp.js').stockWiring({ methods: all });
	}
	...
	// R5 — the instance sharing endpoints (out-of-context: the .sgp rides the wire, never a tool
	// result). Wired when the host passes its residency runtime (`cfg.instances`).
	const instRoutes = cfg.instances ? require('./instances/http.js').instancesRoutes({ runtime: cfg.instances }) : null;

	let srv = null;
	return {
		proxy: px, sync, health, stockLanes,
		...
		start: async function ( onReady ) {
			...
			// owns the socket). Intercept GET /healthz and the /instances routes, delegate the rest
			// to the OpenAI handler.
			const handler = async function ( reqd ) {
				if ( reqd.method === 'GET' && String(reqd.url || '').split('?')[0] === '/healthz' ) {
					return { status: 200, headers: { 'content-type': 'application/json' }, body: health() };
				}
				if ( instRoutes ) {
					const r = await instRoutes(reqd);
					if ( r ) return r;
				}
				return base(reqd);
			};
