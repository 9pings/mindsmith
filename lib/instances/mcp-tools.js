...
/** 'string' / 'string?' / 'any' / … → { schema, required } (fail-closed on an unknown spec).
 * `any` derives an UNCONSTRAINED schema ({}): a descriptor value that is genuinely any JSON —
 * e.g. the `plan` type's `complete.value`, a served answer — must not be forced through a type. */
