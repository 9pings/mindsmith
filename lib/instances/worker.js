...
function bootConf( label ) {
	return { label, isMaster: true, autoMount: true, conceptSets: d.conceptSets || [], bagRefManagers: {},
		logLevel: 'error', maxRevisions: workerData.maxRevisions };
}
...
const ops = {
	...
	revisions: ( p ) => revisionsOf(member(p.graph), p.last),
	mutate: async ( p ) => {
	...
	serialize: async () => {
		...
		for ( const name of Object.keys(graphs) ) {
			...
			views[name] = revisionsOf(graphs[name], 1e9);      // R7: the audit view rides every persist
		}
