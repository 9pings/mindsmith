 ...
 *
 * R7 — the ATTRIBUTED HISTORY survives restarts, bounded: every persist writes
 * `revs/<member>.json` = the audit VIEW ({rev, atoms, by[]} rows — never the raw templates; the
 * facts stay the single truth), merged persisted∪live (one continuous rev space) and kept to the
 * last `revisionsTail` rows (default 2500 — ALIGNED with maxRevisions on purpose: a smaller tail would silently shrink the audit on remount; never cap below the live window without owning that big picture) with an honest `compactedBelow` marker. The HOT graph is bounded by
 * the engine-native `maxRevisions` window (default 2500 — generous on purpose, and tunable via
 * the mindsmith config `instances.maxRevisions`). Contract: `maxRevisions` must exceed the
 * writes between two persist points (like idleTTL vs the longest act) or live view rows are
 * pruned before they are ever persisted. Additive format — a pre-R7 pack still opens.
 */
const fs = require('fs');
...
function createStore( opts ) {
	...
	const revisionsTail = opts.revisionsTail == null ? 2500 : opts.revisionsTail;
	const maxRevisions = opts.maxRevisions == null ? 2500 : opts.maxRevisions;
	...
	function bootConf( label, d ) {
		return { label, isMaster: true, autoMount: true, conceptSets: d.conceptSets || [], bagRefManagers: {}, logLevel: 'error', maxRevisions };
	}
	/** persisted∪live audit rows — live wins by rev; tail-bounded with the honest marker. */
	function mergeRevRows( oldRows, liveRows, prevCompacted, tail ) {
		const byRev = new Map();
		(oldRows || []).forEach(( r ) => byRev.set(r.rev, r) );
		(liveRows || []).forEach(( r ) => byRev.set(r.rev, r) );
		let rows = [...byRev.values()].sort(( a, b ) => a.rev - b.rev );
		let compactedBelow = prevCompacted || 0;
		if ( tail > 0 && rows.length > tail ) {
			rows = rows.slice(-tail);
			compactedBelow = rows[0].rev;
		}
		return { rows, compactedBelow };
	}
	...
	/** Quiescent serialized records + audit views for every member — worker call or local (both settle first). */
	async function serializeAll( h ) {
		if ( h.worker ) return h.worker.call('serialize');
		const records = {}, revs = {}, views = {};
		for ( const name of Object.keys(h.graphs) ) {
			...
			views[name] = revisionsOf(h.graphs[name], 1e9);
		}
		return { records, revs, views };
	}
	async function persist( h ) {
		const { records, revs, views } = await serializeAll(h);
		h.meta.graphs = {};
		...
		// R7 — the audit view rides the pack, merged with what was already persisted (tail-bounded)
		h.revView = h.revView || {};
		for ( const name of Object.keys(records) ) {
			const prev = h.revView[name] || { rows: [], compactedBelow: 0 };
			h.revView[name] = mergeRevRows(prev.rows, (views && views[name] || {}).revisions, prev.compactedBelow, revisionsTail);
			entries.push({ name: 'revs/' + name + '.json', data: JSON.stringify(h.revView[name]) });
		}
	...
	const store = {
		...
		async create( type, o ) {
			...
			const inst = await createInstance(t.descriptor, { seed: o.seed, conceptMap: t.conceptMap, label: id, conf: { maxRevisions } });
		...
		async addGraph( id, name, o ) {
			...
			if ( h.worker ) await h.worker.call('addGraph', { name, seed: o.seed });
				...
				const inst = await createInstance(t.descriptor, { seed: o.seed, conceptMap: t.conceptMap, label: id + '/' + name, conf: { maxRevisions } });
		...
		async open( id ) {
			...
			const p = (async () => {
				...
				const records = {}, revView = {};
				for ( const name of Object.keys(man.graphs || {}) ) {
					records[name] = JSON.parse(readEntry(bytes, 'graphs/' + name + '.json'));
					try { revView[name] = JSON.parse(readEntry(bytes, 'revs/' + name + '.json')); }
					catch ( e ) { /* pre-R7 pack — additive format, the view degrades to live-only */ }
				}
				let h;
				if ( inWorker ) {
					const wh = await spawnInstanceWorker({ descriptorPath: t.path, conceptMap: t.conceptMap, members: records, label: id, maxRevisions });
					wh.onExit(() => { const cur = hot.get(id); if ( cur && cur.worker === wh ) hot.delete(id); });
					h = { worker: wh, descriptor: t.descriptor, meta: man, revView };
				}
				else {
					const graphs = {};
					for ( const name of Object.keys(records) ) {
					...
					h = { graphs, descriptor: t.descriptor, meta: man, revView };
				}
				hot.set(id, h);
				return h;
			})();
		 ...
		 * R7: the live window is MERGED with the persisted audit tail (revs/<member>.json), so
		 * who-wrote-what survives restarts; `compactedBelow` marks where older history was folded.
		 */
		async revisions( id, o ) {
			o = o || {};
			const h = await store.open(id);
			const live = h.worker
				? await h.worker.call('revisions', { graph: o.graph, last: 1e9 })
				: revisionsOf(memberOf(h, o.graph), 1e9);
			const persisted = (h.revView || {})[o.graph || MASTER] || { rows: [], compactedBelow: 0 };
			const merged = mergeRevRows(persisted.rows, live.revisions, persisted.compactedBelow, 0);
			const n = o.last == null ? 20 : Math.max(1, o.last | 0);
			return { current: live.current, snapshots: live.snapshots,
				compactedBelow: merged.compactedBelow, revisions: merged.rows.slice(-n) };
		},
		...
		async fork( id, ctx ) {
			...
			const { records, revs, views } = await serializeAll(parent);
			...
			// the audit history up to the fork point travels with the child (provenance)
			const pv = (parent.revView || {})[MASTER] || { rows: [], compactedBelow: 0 };
			const childView = mergeRevRows(pv.rows, (views && views[MASTER] || {}).revisions, pv.compactedBelow, revisionsTail);
			fs.writeFileSync(file(childId), packEntries([
				...
				{ name: 'revs/master.json', data: JSON.stringify(childView) }
			]));
		...
		async import( bytes, ctx ) {
			...
			for ( const name of Object.keys(man.graphs || {}) ) {
				...
				try { entries.push({ name: 'revs/' + name + '.json', data: readEntry(bytes, 'revs/' + name + '.json') }); }
				catch ( e ) { /* pre-R7 pack — nothing to carry */ }
			}
