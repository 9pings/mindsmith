 ...
 * the engine-native `maxRevisions` window. Contract: `maxRevisions` must exceed the writes
 * between two persist points (like idleTTL vs the longest act) or live view rows are pruned
 * before they are ever persisted. Additive format — a pre-R7 pack (no revs entry) still opens.
 */
const fs = require('fs');
...
function createStore( opts ) {
	...
	const maxRevisions = opts.maxRevisions == null ? 500 : opts.maxRevisions;
