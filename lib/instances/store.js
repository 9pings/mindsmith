 ...
 *
 * Two persist policies: `on-write` (default — every write act persists; right for sequential
 * hosts) and `managed` — acts never persist; the RESIDENCY layer above (runtime.js) persists on
 * the engine's own `stabilize` (quiescent) via sync/evict/close. Under concurrent agents managed
 * is the CORRECT mode, not an optimization: per-write persistence can serialize a
 * mid-stabilization state.
 *
 * FINDABILITY (owner 07-18): an LLM retrieves instances by text/theme — the manifest carries
 * `description` + `tags` (set at create via `meta`, edited via `setMeta`, attributed), and
 * `search({q, tags})` matches id/title/description/tags — manifest-only, never inflating a blob.
 */
const fs = require('fs');
...
function createStore( opts ) {
	...
	const warming = new Map();                               // id -> in-flight open() promise (parallel agents boot ONCE)
	...
	const store = {
		async create( type, o ) {
			...
			const h = {
				...
				meta: {
					...
					title: m.title || (o.seed && o.seed.title) || '',
					description: m.description || '', tags: Array.isArray(m.tags) ? m.tags : [],
					by: o.agent, createdAt: clock(), graphs: {},
		...
		async open( id ) {
			...
			if ( warming.has(id) ) return warming.get(id);     // parallel agents share ONE boot
			const p = (async () => {
		...
		/** Persist a HOT instance now (the residency layer's checkpoint call). */
		async persistNow( id ) {
			const h = hot.get(id);
		...
		async act( id, action, args, ctx ) {
			...
			if ( !managed && a && a.write && r && r.ok ) persist(h);
			return r;
		},
